...
Guiar a implementação e revisão contínuas de funcionalidades dentro de módulos existentes do frontend Web (Next.js App Router, `apps/web/src/{app,modules,shared}`) por meio da skill `frontend-module-workflow`, cobrindo estrutura/empacotamento, fronteiras arquiteturais, decisão de posse de componentes, roteamento para skills complementares, fluxo de trabalho em fases, guardrails de rotas/autenticação, referências especializadas e validação/aceitação.
