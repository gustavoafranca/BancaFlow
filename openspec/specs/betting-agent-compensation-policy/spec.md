...
Define the compensation policy (`CompensationPolicy`) required at `BettingAgent` creation — the mandatory initial policy, the three approved policy types, and the history-compatible storage structure used to persist it.
