...
- **THEN** o sistema retorna `401` com mensagem genérica sem revelar se o username ou a banca existem, e incrementa `failedLoginAttempts`
...
O sistema SHALL contar falhas de login por conta usando `failedLoginAttempts`, `failedLoginWindowStartedAt` e `lockedUntil`, bloqueando automaticamente após 5 falhas dentro de 15 minutos.
...
- **THEN** `failedLoginAttempts = 0`, `failedLoginWindowStartedAt = null` e `lockedUntil = null`
